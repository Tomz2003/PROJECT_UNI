...
def image_inverse(img_path):
    ...
    im_inv = 255 - im_arr
    new_img = Image.fromarray(im_inv.astype(np.uint8)) )
...
def gamma_correction(img_path, gamma=0.5):
    ...
    im_gamma = np.clip(im_gamma, 0, 255).astype(np.uint8)
...
def main_menu_2():
    ...
    while True:
        try:
            ...
            if not img_path:
                img_path = 'ha-long-bay-in-vietnam.jpg'
